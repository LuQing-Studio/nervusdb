...
| T5 | Fuck-off Test：`kill -9` 下的数据一致性验证 | L3 | P0 | Plan | - | 目标：重启后要么事务前要么事务后；校验字典/索引引用一致 |
| T6 | 冻结并对齐 `nervusdb.h`：最小稳定 C 契约（含 Cypher 执行） | L3 | P0 | WIP | feat/T6-ffi-freeze | 收口导出符号；补齐 resolve/exec_cypher/version；写清内存释放规则 |
