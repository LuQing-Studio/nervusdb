...
| T51 | v2 M3：Executor MVP（基于 GraphSnapshot 的流式算子） | L3 | P0 | WIP | feat/T51-v2-executor-mvp | 定义最小算子集（Scan/Expand/Filter/Project/Limit），Pull-based streaming；必要时扩展 `nervusdb-v2-api` 的 snapshot 能力（node scan / id resolve） |
