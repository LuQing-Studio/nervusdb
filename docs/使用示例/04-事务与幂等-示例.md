...
```ts
const db = await SynapseDB.open('orders.synapsedb', {
  enableLock: true,
...
```bash
synapsedb txids orders.synapsedb --list=10
```

### 4. 崩溃恢复演练

1. 在 `commitBatch` 前故意抛错并 `abortBatch`，观察 WAL 中只有 BEGIN/ABORT
2. 手动复制数据库文件，删除 `orders.synapsedb.wal`
3. 恢复后执行 `synapsedb stats`，数据完整

### 5. 清空事务注册表（谨慎）

```bash
synapsedb txids orders.synapsedb --clear
```
