...
// 创建搜索索引
await searchEngine.createIndex('documents',
...
```typescript
// 布尔查询
const booleanResults = await searchEngine.search('documents',
  '人工智能 AND 机器学习 NOT 深度学习'
);
...
edges.forEach(([source, target]) => {
  graph.addEdge({
    source, target, type: 'link', weight: 1.0
  });
});
...
class KnowledgeGraph {
  ...
  async initialize() {
    ...
    await this.searchEngine.createIndex('entities',
