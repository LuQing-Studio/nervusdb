...
const multiLine: MultiLineString = {
  ...
  coordinates: [
    [[116.4, 39.9], [117.2, 39.1]],  // 线段1
    [[117.2, 39.1], [121.5, 31.2]]   // 线段2
  ]
};
...
const detailedLine: LineString = {
  ...
  coordinates: [
    ...
    [116.43, 39.92], [116.44, 39.92], [116.45, 39.93],
    [116.46, 39.93], [116.47, 39.94], [116.48, 39.95]
  ]
};
...
class SpatialIndex {
  ...
  constructor(gridSize: number = 0.01) {  // 0.01度的网格
...
class GeoFence {
  ...
  // 获取进入/离开事件
  checkMovement(previousPoint: Point, currentPoint: Point): 'enter' | 'exit' | 'inside' | 'outside' {
...
class RouteAnalyzer {
  ...
  checkObstacles(route: LineString, obstacles: Polygon[]): Polygon[] {
    return obstacles.filter(obstacle =>
      this.spatial.intersects(route, obstacle)
    );
  }
  ...
  private calculateTurnAngle(p1: [number, number], p2: [number, number], p3: [number, number]): number {
