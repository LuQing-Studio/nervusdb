...
SynapseDB 采用插件架构来扩展功能，所有高级查询能力（路径查找、聚合、Cypher）均通过插件实现。插件系统的设计遵循"好品味"原则：用户无需关心插件加载细节，开箱即用。

## 默认插件

### 自动加载的插件

执行 `SynapseDB.open()` 时，以下插件会自动加载：
...
```ts
// 旧方式（仍然有效）
import { CoreSynapseDB, ExtendedSynapseDB } from 'synapsedb';

const db1 = await CoreSynapseDB.open('demo.synapsedb');
const db2 = await ExtendedSynapseDB.open('demo.synapsedb');
...
**注意**：`CoreSynapseDB` 和 `ExtendedSynapseDB` 只是 `SynapseDB` 的别名，没有功能差异。所有插件在打开数据库时已自动加载。
...
```ts
import { SynapseDBPlugin, PersistentStore, SynapseDB } from 'synapsedb';

export class MyCustomPlugin implements SynapseDBPlugin {
  name = 'my-custom';
  version = '1.0.0';
  ...
  async initialize(db: SynapseDB, store: PersistentStore): Promise<void> {
    this.db = db;
    this.store = store;
    // 初始化逻辑
  }
...
```ts
import { SynapseDB, PluginManager } from 'synapsedb';
...
**正确做法**：贡献到 `src/plugins/` 并在 `SynapseDB.open()` 中默认加载。
...
### Q3: 旧代码使用 `CoreSynapseDB`，需要迁移吗？

**A**: 不需要。别名永久保持向后兼容。但新代码推荐使用 `SynapseDB` 以保持简洁。
