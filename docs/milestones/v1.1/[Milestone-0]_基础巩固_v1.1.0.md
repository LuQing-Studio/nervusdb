...
- [ ] 修改 `PagedIndexReader` 支持游标式读取
- [ ] 实现 `QueryBuilder` 异步迭代器协议
- [ ] 添加 `take()`, `skip()`, `batch()` 方法
...
- [ ] `PropertyInvertedIndex` 类实现
- [ ] 索引持久化机制
- [ ] 写入时自动更新索引

**第7-8周：查询集成**

- [ ] `whereProperty()` API 设计
- [ ] 查询优化器集成
- [ ] 性能测试与调优
...
- [ ] `src/storage/propertyIndex.ts` - 属性倒排索引
- [ ] `src/graph/labels.ts` - 节点标签系统
- [ ] `src/graph/paths.ts` - 变长路径查询
- [ ] `src/query/aggregation.ts` - 聚合函数框架
- [ ] `benchmarks/` - 性能基准测试套件
