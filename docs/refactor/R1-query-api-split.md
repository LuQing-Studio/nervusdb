...
- 已完成切片 2：`strip_explain_prefix` 抽取到
  ...
  并补 3 个单元测试。
- 已完成切片 3：删除/创建校验簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/write_validation.rs`，
...
- 切片 3 回归结果：
  ...
  - `cargo test -p nervusdb-v2 --test create_test` 通过。
  - `cargo test -p nervusdb-v2 --test t304_remove_clause` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
  - `cargo test -p nervusdb-v2 --test t333_varlen_direction` 通过。
- 已完成切片 4：写入计划编译簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/write_compile.rs`，
...
- 已完成切片 5：`CREATE/MERGE` 计划编译簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/write_create_merge.rs`，
...
- 已完成切片 6：`MATCH` 重锚定/可选解绑辅助簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/match_anchor.rs`，
  包含 `maybe_reanchor_pattern / first_relationship_is_bound / build_optional_unbind_aliases` 及私有 helper。
- 已完成切片 7：模式谓词防护簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/pattern_predicate.rs`，
  包含 `ensure_no_pattern_predicate / contains_pattern_predicate`。
- 已完成切片 8：`MERGE` set-item 辅助簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/merge_set.rs`，
  包含 `extract_merge_pattern_vars / compile_merge_set_items`。
- 已完成切片 9：表达式类型校验簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/type_validation.rs`，
  包含 `validate_expression_types / is_definitely_non_boolean / is_definitely_non_list_literal`。
- 已完成切片 10：`WHERE` 绑定与作用域校验簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/where_validation.rs`，
...
- 切片 5/6 回归结果：
  - `cargo test -p nervusdb-v2-query --lib` 通过。
  - `cargo test -p nervusdb-v2 --test create_test` 通过。
  - `cargo test -p nervusdb-v2 --test merge_test` 通过。
  - `cargo test -p nervusdb-v2 --test t323_merge_semantics` 通过。
  - `cargo test -p nervusdb-v2 --test t151_optional_match` 通过。
  - `cargo test -p nervusdb-v2 --test t333_varlen_direction` 通过。
  - `cargo test -p nervusdb-v2 --test t304_remove_clause` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
- 切片 7 回归结果：
...
- 切片 9 回归结果：
  - `cargo test -p nervusdb-v2-query --lib` 通过。
  - `cargo test -p nervusdb-v2 --test create_test` 通过。
  - `cargo test -p nervusdb-v2 --test t332_binding_validation` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
- 切片 10 回归结果：
  - `cargo test -p nervusdb-v2-query --lib` 通过。
  - `cargo test -p nervusdb-v2 --test t332_binding_validation` 通过。
...
- 已完成切片 11：`WITH/RETURN` 计划编译簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/return_with.rs`，
...
- 已完成切片 12：投影/聚合/排序编译簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/projection_compile.rs`，
...
- 切片 12 回归结果：
  - `cargo test -p nervusdb-v2-query --lib` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
  - `cargo test -p nervusdb-v2 --test t313_functions` 通过。
  - `cargo test -p nervusdb-v2 --test t62_order_by_skip_test` 通过。
  - `cargo test -p nervusdb-v2 --test t332_binding_validation` 通过。
- 已完成切片 13：`MATCH` 计划编译簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/match_compile.rs`，
...
- 切片 13 回归结果：
  ...
  - `cargo test -p nervusdb-v2 --test t333_varlen_direction` 通过。
  - `cargo test -p nervusdb-v2 --test t151_optional_match` 通过。
  - `cargo test -p nervusdb-v2 --test t318_paths` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
- 已完成切片 14：变量绑定推断/校验簇抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/binding_analysis.rs`，
...
- 切片 14 回归结果：
  ...
  - `cargo test -p nervusdb-v2 --test t332_binding_validation` 通过。
  - `cargo test -p nervusdb-v2 --test t151_optional_match` 通过。
  - `cargo test -p nervusdb-v2 --test t324_foreach` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
...
- 已完成切片 15：`FOREACH` 子计划编译抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/foreach_compile.rs`，
...
- 已完成切片 16：写路径判定 helper 抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/plan_introspection.rs`，
...
- 切片 16 回归结果：
  ...
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
- 已完成切片 17：主编译管线抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/compile_core.rs`，
  包含 `CompiledQuery` 与 `compile_m3_plan`，并保持子查询/UNION/FOREACH 递归编译行为不变。
- `query_api.rs` 行数从 `580` 进一步降到 `341`。
- 切片 17 回归结果：
  - `cargo test -p nervusdb-v2-query --lib` 通过。
  - `cargo test -p nervusdb-v2 --test t319_subquery` 通过。
  - `cargo test -p nervusdb-v2 --test t324_foreach` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
...
- 已完成切片 18：`prepare` 入口实现抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/prepare_entry.rs`，
  `query_api.rs` 保留公开 `prepare` 壳函数并委托新模块，EXPLAIN/普通路径行为保持不变。
- `query_api.rs` 行数从 `341` 进一步降到 `306`。
- 切片 18 回归结果：
  - `cargo test -p nervusdb-v2-query --lib` 通过。
  - `cargo test -p nervusdb-v2 --test t52_query_api` 通过。
  - `cargo test -p nervusdb-v2 --test t104_explain_test` 通过。
  - `cargo test -p nervusdb-v2 --test t311_expressions` 通过。
- 已完成切片 19：`PreparedQuery` 实现体抽取到
  `/Volumes/WorkDrive/Code/github.com/LuQing-Studio/rust/nervusdb/nervusdb-v2-query/src/query_api/prepared_query_impl.rs`，
