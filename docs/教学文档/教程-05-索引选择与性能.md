...
await engine.batchIndex([
  { id: 'doc:001', title: 'SynapseDB 指南', body: '链式联想、WAL、属性索引' },
]);
...
- `synapsedb auto-compact` 会尊重活跃读者，避免破坏快照
...
| 大型分析   | `synapsedb dump` 导出并配合外部工具处理            |

## 监控指标

- `synapsedb stats --summary`：页数、墓碑比例、WAL 大小
- `synapsedb stats --txids`：事务速率与来源分布
...
| GC 后文件仍大     | 有 LSM 暂存或 WAL 未清理       | `synapsedb auto-compact --include-lsm-segments`，再执行 `gc` |
