...
- 详细了解 SynapseDB 主文件、分页索引、WAL、热度/读者/事务注册表的结构
...
```
<name>.synapsedb             # 主数据文件
<name>.synapsedb.pages/      # 分页索引目录
<name>.synapsedb.wal         # 写前日志 (Write-Ahead Log)
```
...
- `synapsedb auto-compact` 在有读者时跳过对应主键
...
1. 执行 `synapsedb auto-compact --auto-gc` 保证干净状态
2. 复制 `<db>.synapsedb`、`<db>.synapsedb.pages/`、`<db>.synapsedb.wal`
...
synapsedb dump demo.synapsedb SPO 0
pnpm exec tsx scripts/dump-graph.mjs demo.synapsedb > dump.ndjson
```
...
| 现象           | 分析               | 解决                                                |
| -------------- | ------------------ | --------------------------------------------------- |
| manifest 缺失  | 异常中断或权限问题 | 执行 `synapsedb repair` 或 `synapsedb compact` 重建 |
| orphans 未清理 | GC 未执行或被跳过  | `synapsedb gc` 再次执行，确保无活跃读者             |
| WAL 校验失败   | 文件结尾损坏       | `synapsedb check --strict` 自动截断；保持备份       |
