...
将代码仓库中的文件、符号、依赖、测试覆盖关系写入 SynapseDB，构建可查询的知识图谱，支撑影响分析与文档生成。
...
const db = await SynapseDB.open('code.synapsedb', {
  enableLock: true,
...
- 每次导入使用新 `txId`，失败自动重放
- 定期执行 `synapsedb auto-compact code.synapsedb --auto-gc`
- 使用 `synapsedb stats --txids` 监控导入频率

## 可视化

- 将 `synapsedb dump` 输出的 NDJSON 输入 `Graphviz` 或 `Neo4j Bloom`
