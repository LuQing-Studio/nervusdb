...
npm i -g .                  # 生成 synapsedb 命令（可选）
# 或使用 pnpm：pnpm dlx npm -g install .

synapsedb --help            # 验证 CLI 是否可用
synapsedb --version
```
...
- 默认生成数据库到当前工作目录
- 建议创建专用存储目录：`mkdir -p ~/data/synapsedb`
...
const db = await SynapseDB.open('demo.synapsedb', { enableLock: true });
...
| `synapsedb` 未找到  | 确认 `npm i -g .` 成功并刷新 shell；或使用 `npx synapsedb`     |
| Windows 路径异常    | 使用 WSL 或 PowerShell，路径写成 `C:/data/demo.synapsedb`      |
| build 报错          | 清理 `node_modules`，执行 `pnpm install && pnpm build`         |

## 验证清单

- [ ] `synapsedb --version` 正常输出
- [ ] `synapsedb stats demo.synapsedb` 正常
