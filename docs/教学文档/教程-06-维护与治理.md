...
```bash
synapsedb compact demo.synapsedb --orders=SPO --page-size=1024 --compression=brotli:4
...
synapsedb repair-page demo.synapsedb SPO 123
```
...
synapsedb txids demo.synapsedb --since=60
synapsedb txids demo.synapsedb --session=ingest-service
```
...
set -euo pipefail
DB=$1
synapsedb stats "$DB" --summary > logs/$(date +%F)-stats.log
synapsedb auto-compact "$DB" --mode=incremental --hot-threshold=1.2 --auto-gc
synapsedb txids "$DB" --since=1440 > logs/$(date +%F)-txids.log
```
...
| stats 输出为空          | 路径错误或文件损坏     | 确认数据库路径，执行 `synapsedb check`          |
