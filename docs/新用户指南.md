...
> 欢迎使用 SynapseDB v1.1.0！本指南帮助您快速上手。
...
```bash
# 克隆仓库
git clone https://github.com/your-org/SynapseDB.git
cd SynapseDB
...
```bash
# 使用 pnpm dlx
pnpm dlx synapsedb stats demo.synapsedb
```
...
```bash
# 1. 创建测试数据库（200条记录）
synapsedb bench test.synapsedb 200 default

# 2. 查看统计信息
synapsedb stats test.synapsedb

# 3. 检查健康状态
synapsedb check test.synapsedb

# 4. 清理测试文件
rm -rf test.synapsedb*
```
...
3. **提交 Issue**：[GitHub Issues](https://github.com/your-org/SynapseDB/issues)
...
**总计时间**：约 1 小时，您就可以开始使用 SynapseDB！
