...
- 新增 `nervusdb-core/` Rust crate（位于仓库根目录），负责实现持久化内核、WAL、索引等功能
- TypeScript 层通过 `src/native/core.ts` 中的加载器优雅降级：当原生绑定缺失时继续使用现有纯 TS 实现
- CI 新增 `cargo fmt` 与 `cargo test` 步骤，确保 Rust 核心与 Node 代码共存时的质量
