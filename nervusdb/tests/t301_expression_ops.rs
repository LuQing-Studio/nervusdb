...
#[test]
fn test_arithmetic_in_where_and_set() -> nervusdb::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_in_operator_null_semantics_for_nested_lists() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    ...
    let rows: Vec<_> = nervusdb::query::prepare(q)?
        .execute_streaming(&snapshot, &Params::default())
        .collect::<Result<Vec<_>, _>>()?;
    assert_eq!(rows.len(), 1);
...
fn test_in_operator_rejects_non_list_literal_rhs_at_compile_time() {
    ...
    for query in cases {
        let err = nervusdb::query::prepare(query)
...
#[test]
fn test_comparison_list_and_map_null_semantics() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    let db = Db::open(dir.path().join("t301_cmp_nulls.ndb"))?;
    let snapshot = db.snapshot();

    let list_q = "RETURN [null] = [1] AS a, [[1], [2]] = [[1], [null]] AS b";
    let list_rows: Vec<_> = nervusdb::query::prepare(list_q)?
        .execute_streaming(&snapshot, &Params::default())
        .collect::<Result<Vec<_>, _>>()?;
    ...
    let map_rows: Vec<_> = nervusdb::query::prepare(map_q)?
        .execute_streaming(&snapshot, &Params::default())
        .collect::<Result<Vec<_>, _>>()?;
...
#[test]
fn test_comparison_nan_equality_behavior() -> nervusdb::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_large_integer_literal_keeps_precision_in_match() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    let db = Db::open(dir.path().join("t301_bigint_match.ndb"))?;
    let params = Params::new();

    let mut txn = db.begin_write();
    nervusdb::query::prepare("CREATE (:TheLabel {id: 4611686018427387905})")?.execute_write(
        &db.snapshot(),
        &mut txn,
        &params,
    )?;
    ...
    let snapshot = db.snapshot();
    let equal_rows: Vec<_> = nervusdb::query::prepare(
    ...
    let non_equal_rows: Vec<_> = nervusdb::query::prepare(
...
#[test]
fn test_range_comparison_cross_type_returns_null_except_numbers() -> nervusdb::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_full_bound_range_comparison_chain_in_where_clause() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    ...
    {
        let mut txn = db.begin_write();
        nervusdb::query::prepare("UNWIND [1, 2, 3] AS i CREATE (:Num {num: i})")?.execute_write(
            &db.snapshot(),
            &mut txn,
            &params,
        )?;
        nervusdb::query::prepare("UNWIND ['a', 'b', 'c'] AS c CREATE (:Str {name: c})")?
    ...
    let numeric_rows: Vec<_> =
        nervusdb::query::prepare("MATCH (n:Num) WHERE 1 < n.num <= 3 RETURN n.num AS v")?
            .execute_streaming(&snapshot, &params)
            .collect::<Result<Vec<_>, _>>()?;
    ...
    let string_rows: Vec<_> =
        nervusdb::query::prepare("MATCH (n:Str) WHERE 'a' <= n.name < 'c' RETURN n.name AS v")?
            .execute_streaming(&snapshot, &params)
            .collect::<Result<Vec<_>, _>>()?;
    ...
    let empty_rows: Vec<_> =
        nervusdb::query::prepare("MATCH (n:Num) WHERE 10 < n.num <= 3 RETURN n.num AS v")?
            .execute_streaming(&snapshot, &params)
            .collect::<Result<Vec<_>, _>>()?;
...
#[test]
fn test_comparison_chain_with_mixed_operators_reuses_middle_operand() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    ...
    let snapshot = db.snapshot();
    let rows: Vec<_> = nervusdb::query::prepare(
        "MATCH (n)-->(m) \
...
#[test]
fn test_range_comparison_list_and_nan_semantics() -> nervusdb::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_range_comparison_unwind_numeric_pairs() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    let db = Db::open(dir.path().join("t301_range_unwind.ndb"))?;
    let params = Params::new();

    let mut txn = db.begin_write();
    nervusdb::query::prepare("CREATE ()-[:T]->()")?.execute_write(
        &db.snapshot(),
        &mut txn,
        &params,
    )?;
    ...
    let snapshot = db.snapshot();
    let direct_rows: Vec<_> =
        nervusdb::query::prepare("RETURN 1 < 3.14 AS lt, [1, 3.14][0] < [1, 3.14][1] AS idx_lt")?
            .execute_streaming(&snapshot, &params)
            .collect::<Result<Vec<_>, _>>()?;
...
#[test]
fn test_hex_and_octal_integer_literals_roundtrip() -> nervusdb::Result<()> {
    let dir = tempdir()?;
