...
#[test]
fn test_incoming_relationship() -> nervusdb::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_undirected_relationship() -> nervusdb::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t315.ndb");
    let db = Db::open(&db_path)?;
    let mut txn = db.begin_write();
    ...
    let prep1 = nervusdb::query::prepare(query1)?;
    let snapshot = db.snapshot();
    ...
    let prep2 = nervusdb::query::prepare(query2)?;
