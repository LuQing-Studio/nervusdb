...
export async function autoCompact(dbPath, options = {}) {
    ...
    for (const order of orders) {
        ...
        const multiPagePrimaries = [...cnt.entries()].filter(([, pageCount]) => pageCount >= minMergePages);
        const hasMergeCandidate = multiPagePrimaries.length > 0;
        console.log(`   🔗 Unique primaries: ${cnt.size}`);
        console.log(`   📊 Multi-page primaries (>=${minMergePages} pages): ${multiPagePrimaries.length}`);
        ...
        if (options.mode !== 'rewrite' && hot && options.hotThreshold && options.hotThreshold > 0) {
            ...
            for (const [pval, count] of cnt.entries()) {
                ...
                if (hotCount >= options.hotThreshold && score >= minScore) {
                    ...
                    console.log(`      • Tombstone factor: ${tombTerm} (score: +${scoreDetail.score.tombstone})`);
                    console.log(`      • Total score: ${scoreDetail.score.total}`);
                    console.log(`      • Action: INCLUDE`);
                    console.log(`      • Reason: score >= ${minScore} AND hotness >= ${options.hotThreshold}`);
                    ...
                    if (score < minScore)
                        reasons.push(`score ${score} < ${minScore}`);
            ...
            if (topK.length > 0) {
                ...
                topK.forEach((c, i) => {
                    console.log(`      ${i + 1}. Primary ${c.p}: hotness=${c.c}, pages=${c.pages}, score=${c.score}`);
                });
    ...
    if (!includeLsmSegments && options.includeLsmSegmentsAuto) {
        try {
            ...
            if (segs >= segTh || triples >= triTh) {
                ...
                const reasons = [];
                if (segs >= segTh)
                    reasons.push(`segments ${segs} >= ${segTh}`);
                if (triples >= triTh)
                    reasons.push(`triples ${triples} >= ${triTh}`);
