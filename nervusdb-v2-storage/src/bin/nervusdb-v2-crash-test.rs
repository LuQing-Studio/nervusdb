...
//!
//! Usage:
//!   cargo run -p nervusdb-v2-storage --bin nervusdb-v2-crash-test -- driver <path> [--iterations N]
//!   cargo run -p nervusdb-v2-storage --bin nervusdb-v2-crash-test -- writer <path> [--batch N]
//!   cargo run -p nervusdb-v2-storage --bin nervusdb-v2-crash-test -- verify <path>
//!
//! Note: <path> is a base path; it will use <path>.ndb and <path>.wal.

use nervusdb_v2_storage::csr::CsrSegment;
use nervusdb_v2_storage::engine::GraphEngine;
use nervusdb_v2_storage::pager::Pager;
use nervusdb_v2_storage::wal::{SegmentPointer, Wal, WalRecord};
...
fn main() -> ExitCode {
    ...
    {
        eprintln!("nervusdb-v2-crash-test is not supported on wasm32 targets");
...
fn print_usage() {
    eprintln!(
        "Usage:\n  nervusdb-v2-crash-test driver <path> [--iterations N] [--min-ms A] [--max-ms B] [--batch N] [--node-pool N] [--rel-pool N]\n  nervusdb-v2-crash-test writer <path> [--batch N] [--node-pool N] [--rel-pool N] [--compact-every N] [--seed S]\n  nervusdb-v2-crash-test verify <path> [--node-pool N]\n\nNote: <path> is a base path; it will use <path>.ndb and <path>.wal."
    );
}
