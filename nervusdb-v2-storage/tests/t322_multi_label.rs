use nervusdb_v2_storage::idmap::{IdMap, I2eRecord};
...
fn test_multi_label_create_and_lookup() {
    ...
    idmap.apply_create_node_multi_label(&mut pager, 100, labels.clone(), 0).unwrap();
...
fn test_add_remove_labels() {
    ...
    idmap.apply_create_node_multi_label(&mut pager, 100, vec![1], 0).unwrap();
...
fn test_persistence_multi_label() {
    ...
    
    {
        let mut pager = Pager::open(&ndb).unwrap();
        let mut idmap = IdMap::load(&mut pager).unwrap();
        idmap.apply_create_node_multi_label(&mut pager, 100, vec![1, 2, 3], 0).unwrap();
    }
