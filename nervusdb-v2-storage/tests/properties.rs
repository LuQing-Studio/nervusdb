...
use nervusdb_v2_storage::Result;
...
fn test_node_property_set_and_get() -> Result<()> {
    ...
    // Read transaction: verify properties
    {
        ...
        assert_eq!(
            name,
            Some(nervusdb_v2_api::PropertyValue::String("Alice".to_string()))
        );

        let age = api_snapshot.node_property(0, "age");
        assert_eq!(age, Some(nervusdb_v2_api::PropertyValue::Int(30)));
        ...
        assert_eq!(
            props.get("age"),
            Some(&nervusdb_v2_api::PropertyValue::Int(30))
        );
    }
...
fn test_edge_property_set_and_get() -> Result<()> {
    ...
    {
        let api_snapshot = engine.snapshot();
        let edge = nervusdb_v2_api::EdgeKey {
            src: 0,
            rel: 1,
            dst: 1,
        };
        ...
        assert_eq!(weight, Some(nervusdb_v2_api::PropertyValue::Float(0.5)));
    }
...
fn test_wal_replay_properties() -> Result<()> {
    ...
    // Reopen and verify properties are recovered
    {
        ...
        assert_eq!(
            name,
            Some(nervusdb_v2_api::PropertyValue::String("Bob".to_string()))
        );
    }
...
fn test_multiple_property_types() -> Result<()> {
...

    {
        ...
        assert_eq!(
            ...
            Some(nervusdb_v2_api::PropertyValue::Null)
        );
        assert_eq!(
            api_snapshot.node_property(0, "bool"),
            Some(nervusdb_v2_api::PropertyValue::Bool(true))
        );
        assert_eq!(
            api_snapshot.node_property(0, "int"),
            Some(nervusdb_v2_api::PropertyValue::Int(42))
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(nervusdb_v2_api::PropertyValue::String("hello".to_string()))
        );
    }
