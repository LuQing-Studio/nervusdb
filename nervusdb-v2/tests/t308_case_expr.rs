...
#[test]
fn test_case_when_then() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_case_else() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t308.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_case_multiple_when() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t308.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_case_with_property() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t308.ndb");
    let db = Db::open(&db_path)?;
    ...
    txn.set_node_property(n, "age".to_string(), nervusdb_v2::PropertyValue::Int(25))?;
    txn.commit()?;
