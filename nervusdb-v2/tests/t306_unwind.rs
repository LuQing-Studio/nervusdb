...
#[test]
fn test_unwind_list() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_unwind_empty_list() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t306.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_unwind_null() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t306.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_unwind_scalar() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t306.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_unwind_chaining() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t306.ndb");
    let db = Db::open(&db_path)?;
    ...
    txn.set_node_property(
        node,
        "nums".to_string(),
        nervusdb_v2::PropertyValue::List(vec![
            nervusdb_v2::PropertyValue::Int(10),
            nervusdb_v2::PropertyValue::Int(20),
        ]),
    )?;
    txn.commit()?;
