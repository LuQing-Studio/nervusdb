...
#[test]
fn test_union_distinct() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
...
#[test]
fn test_union_dedup() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t307.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_union_all() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t307.ndb");
    let db = Db::open(&db_path)?;
...
#[test]
fn test_union_with_match() -> nervusdb_v2::Result<()> {
    let dir = tempdir()?;
    let db_path = dir.path().join("t307.ndb");
    let db = Db::open(&db_path)?;
    ...
    txn.set_node_property(n1, "id".to_string(), nervusdb_v2::PropertyValue::Int(100))?;
    let n2 = txn.create_node(label_b.into(), 0)?;
    txn.set_node_property(n2, "id".to_string(), nervusdb_v2::PropertyValue::Int(200))?;
    txn.commit()?;
