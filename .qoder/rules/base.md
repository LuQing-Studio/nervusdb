...
- Repomix（代码仓分析/片段检索）：本地/远程仓库结构分析、打包、片段搜索与读取大文件局部内容。
- Playwright MCP（浏览器自动化/交互）：网页交互验证或动态内容只读截取；敏感/有副作用操作需事先授权。
- MySQL（数据库只读为主）：结构检查与 SELECT 查询；写操作需明确授权并使用事务与最小权限账户。
...
│  │  │  ├─ src/query/path/astar.ts
│  │  │  ├─ src/query/path/bidirectional.ts
│  │  │  ├─ src/query/path/bidirectionalSimple.ts
│  │  │  ├─ src/query/path/variable.ts  变长路径构建器
│  │  └─ src/query/pattern  模式匹配/
│  │     ├─ src/query/pattern/ast.ts
│  │     ├─ src/query/pattern/compiler.ts
│  │     ├─ src/query/pattern/executor.ts
│  │     ├─ src/query/pattern/index.ts
│  │     ├─ src/query/pattern/lexer.ts
│  │     ├─ src/query/pattern/match.ts  模式匹配执行
│  │     ├─ src/query/pattern/parser.ts
│  │     └─ src/query/pattern/planner.ts
