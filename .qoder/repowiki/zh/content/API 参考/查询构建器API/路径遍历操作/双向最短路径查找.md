...
<cite>
**本文档引用文件**  
- [bidirectional.ts](file://src/query/path/bidirectional.ts) - *核心算法实现*
- [bidirectionalSimple.ts](file://src/query/path/bidirectionalSimple.ts) - *简化版本实现*
- [synapseDb.ts](file://src/synapseDb.ts) - *API 入口层*
- [pathfinding.ts](file://src/plugins/pathfinding.ts) - *插件接口层*
- [shortest_path_bidirectional.test.ts](file://tests/integration/query/path/shortest_path_bidirectional.test.ts) - *集成测试用例*
</cite>

## 更新摘要
**变更内容**  
- 新增 `shortestPathBidirectional` API 入口说明，明确其在 `SynapseDB` 类中的位置
- 更新核心实现机制部分，补充插件架构调用链路
- 增强与传统单向BFS对比优势的性能分析
- 修正路径拼接逻辑描述，确保与最新代码一致
- 更新所有受影响的源码引用链接及注释
...
在架构设计上，`SynapseDB` 类通过插件系统调用 `PathfindingPlugin` 提供的 `shortestPathBidirectional` 方法，形成三层调用链：
1. **API层**：`SynapseDB.shortestPathBidirectional()`
2. **插件层**：`PathfindingPlugin.shortestPathBidirectional()`
3. **算法层**：`BidirectionalPathBuilder.shortestPath()`
...
**Section sources**  
- [synapseDb.ts](file://src/synapseDb.ts#L544-L555) - *API 入口方法*
- [pathfinding.ts](file://src/plugins/pathfinding.ts#L104-L257) - *插件接口实现*
- [bidirectional.ts](file://src/query/path/bidirectional.ts#L30-L269) - *核心算法类*
