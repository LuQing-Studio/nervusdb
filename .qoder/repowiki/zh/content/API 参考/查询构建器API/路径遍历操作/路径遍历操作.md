...
<cite>
**本文档引用的文件**
- [queryBuilder.ts](file://src/query/queryBuilder.ts) - *在最近的提交中更新了惰性执行和流式处理功能*
...
- [queryEngine.ts](file://src/storage/managers/queryEngine.ts) - *实现流式查询引擎核心逻辑*
</cite>

## 更新摘要
**变更内容**
- 更新了 `followPath` 方法的实现机制，区分了 EAGER 和 LAZY 两种执行模式
- 新增了惰性查询计划的基数估算功能，包含执行阶段分析
- 更新了相关代码示例以反映流式处理行为
- 增强了性能优化策略部分，包含惰性执行的流式优势
- 引入基于 `streamFactRecords` 的真·流式扩展机制，确保前沿节点与边的去重及顺序稳定
...
系统现在支持两种执行模式：EAGER（立即执行）和LAZY（惰性执行）。EAGER模式保持原有的同步BFS语义，而LAZY模式采用流式处理，通过查询计划实现内存高效的路径遍历。
...
**代码来源**
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L707-L775) - *EAGER 模式的 followPath 实现*
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L1899-L1941) - *LAZY 模式的 followPath 实现*
...
5. LAZY模式下支持真正的流式处理，避免大结果集的内存峰值
6. 查询计划包含基数估算，可预测执行阶段的输出规模
7. 基于 `streamFactRecords` 的流式扩展机制，保证前沿节点与边的去重及顺序稳定
...
LazyStreaming["惰性流式处理"]
CardinalityEstimation["基数估算"]
end
MinHeap --> Dijkstra
Cache --> Bidirectional
...
LazyStreaming --> Performance
CardinalityEstimation --> Performance
```
...
- [astar.ts](file://src/query/path/astar.ts#L0-L344)
- [bidirectional.ts](file://src/query/path/bidirectional.ts#L0-L301)
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L1899-L1941) - *惰性执行和基数估算*
- [persistentStore.ts](file://src/storage/persistentStore.ts#L306-L315) - *streamFactRecords 流式查询*
- [queryEngine.ts](file://src/storage/managers/queryEngine.ts#L92-L100) - *流式查询引擎实现*