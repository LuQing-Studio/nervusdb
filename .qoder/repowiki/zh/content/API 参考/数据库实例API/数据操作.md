...
<cite>
**本文档引用的文件**
- [synapseDb.ts](file://src/synapseDb.ts) - *更新了查询执行模型*
- [persistentStore.ts](file://src/storage/persistentStore.ts)
- [wal.ts](file://src/storage/wal.ts)
- [queryBuilder.ts](file://src/query/queryBuilder.ts) - *新增流式查询支持*
...
## 更新摘要
**变更内容**
- 更新了 `find` 方法的执行模型，从即时物化（EAGER）改为默认惰性执行（LAZY）
- 新增了 `findStreaming` 方法的详细说明及其与 `StreamingQueryBuilder` 的交互
- 修正了数据读取与一致性章节中关于查询执行行为的描述
- 更新了相关代码示例和流程图以反映最新的惰性执行机制
- 增加了对 `buildStreamingFindContext` 函数的源码分析
...
1.  **find**：作为查询起点，根据条件创建初始的 `QueryBuilder` 实例，包含匹配的事实记录和前沿节点集（`frontier`）。**注意**：自 `50829022` 提交后，`find()` 默认返回 `LazyQueryBuilder`，采用惰性执行模型，仅在实际迭代时逐步产出结果。
...
*   **惰性执行与快照语义**：在 `withSnapshot` 回调内，`find()` 会回落为“即刻物化”模式，确保整个查询链路在固定的 `epoch` 上执行，保证快照一致性。

**Section sources**
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L38-L812)
- [synapseDb.ts](file://src/synapseDb.ts#L206-L216) - *更新了find方法的惰性执行逻辑*
- [synapseDb.ts](file://src/synapseDb.ts#L400-L446)
- [persistentStore.ts](file://src/storage/wal.ts#L1-L419)

### 流式查询与内存效率
为了支持大规模数据集的高效查询，系统引入了 `findStreaming` API：

1.  **findStreaming方法**：该方法返回一个 `StreamingQueryBuilder`，它通过 `buildStreamingFindContext` 异步构建查询上下文，并利用 `streamFactRecords` 接口实现真正的流式迭代。
2.  **buildStreamingFindContext**：此函数创建一个异步生成器 `streamFacts`，它按批次从磁盘流式读取数据，同时动态构建前沿节点集（`frontier`），避免一次性加载所有数据到内存。
3.  **StreamingQueryBuilder**：该类实现了 `AsyncIterator` 接口，允许用户使用 `for await...of` 语法逐条处理结果，极大降低了内存占用。

```mermaid
flowchart TD
StartStream([开始 findStreaming]) --> GetEpoch["获取当前Epoch"]
GetEpoch --> PushEpoch["将Epoch压入pinnedEpochStack"]
PushEpoch --> BuildContext["异步构建流式查询上下文"]
BuildContext --> CreateStream["创建AsyncIterableIterator流"]
CreateStream --> ReturnQB["返回StreamingQueryBuilder"]
ReturnQB --> Iterate["for await (const fact of qb)"]
Iterate --> Yield["逐条产出FactRecord"]
Yield --> Continue["继续下一条"]
Continue --> Iterate
Iterate --> Done["迭代完成"]
Done --> PopEpoch["从pinnedEpochStack弹出Epoch"]
PopEpoch --> End["结束"]
```

**Diagram sources**
- [synapseDb.ts](file://src/synapseDb.ts#L228-L243) - *findStreaming方法实现*
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L1943-L2000) - *buildStreamingFindContext函数*
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L882-L971) - *StreamingQueryBuilder类*

**Section sources**
- [synapseDb.ts](file://src/synapseDb.ts#L228-L243)
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L1943-L2000)
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L882-L971)
- [persistentStore.ts](file://src/storage/persistentStore.ts#L306-L315)
- [queryEngine.ts](file://src/storage/managers/queryEngine.ts#L55-L69)
