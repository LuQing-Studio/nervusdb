...
- **执行池策略**：使用 `pool: 'forks'` 并设置 `singleFork: true`，避免多进程并发争用文件句柄和内存资源，减少 OOM 风险。
- **串行执行**：`sequence.concurrent: false` 禁用并发测试执行，进一步降低资源竞争。
...
这种分层组织方式使得开发者可以快速找到相关测试，并根据需要运行特定子集。
