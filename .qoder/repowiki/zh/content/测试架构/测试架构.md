# 测试架构

<cite>
**本文档引用的文件**
- [vitest.config.ts](file://vitest.config.ts) - *已更新测试配置*
- [minHeap.ts](file://src/utils/minHeap.ts)
- [staging.ts](file://src/storage/staging.ts)
- [minHeap.test.ts](file://tests/unit/storage/minHeap.test.ts)
- [staging.test.ts](file://tests/unit/storage/staging.test.ts)
- [query.test.ts](file://tests/integration/query/aggregation.test.ts)
- [storage.test.ts](file://tests/integration/storage/wal.test.ts)
- [system.test.ts](file://tests/system/query_snapshot_isolation.test.ts)
- [performance.test.ts](file://tests/performance/performance_baseline.test.ts)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml) - *CI流程优化*
- [tempfs.ts](file://tests/helpers/tempfs.ts) - *新增测试助手工具*
- [rtree.ts](file://src/spatial/rtree.ts) - *修复插入分裂回溯越界问题*
- [rtree.test.ts](file://tests/unit/spatial/rtree.test.ts) - *R-Tree基础查询测试*
- [spatialQuery.test.ts](file://tests/unit/spatial/spatialQuery.test.ts) - *空间查询管理器测试*
- [global-cleanup.ts](file://tests/setup/global-cleanup.ts) - *新增全局清理模块*
- [snapshot_memory_optimization.test.ts](file://tests/system/snapshot_memory_optimization.test.ts) - *调整内存测试阈值*
</cite>

## 更新摘要
**变更内容**
- 根据最新提交修复了因算法死循环导致测试挂起的问题
- 测试套件已拆分以提升可维护性，文档结构调整以反映新组织方式
- CI工作流优化，测试执行策略调整
- 更新Vitest配置说明，准确描述并发控制与资源管理机制
- 引入 `tempfs` 测试助手模块，统一管理临时目录创建与清理
- 修复R-Tree索引在插入时的分裂回溯越界问题，确保沿插入路径自底向上检查分裂操作的安全性
- **新增全局清理机制**：通过 `global-cleanup.ts` 在测试结束后自动移除临时数据库文件
- **强化CI检查**：在CI流程中增加对临时文件残留的断言，确保构建环境的纯净
- **调整内存测试阈值**：为 `snapshot_memory_optimization.test.ts` 中的内存增长验收标准提供环境自适应性

### 目录
1. [引言](#引言)
2. [测试分层策略](#测试分层策略)
3. [Vitest 框架配置与组织结构](#vitest-框架配置与组织结构)
4. [编写新的测试用例](#编写新的测试用例)
5. [测试驱动开发的重要性](#测试驱动开发的重要性)
6. [结论](#结论)

## 引言
SynapseDB 是一个高性能图数据库系统，其质量保障体系依赖于一套全面且分层的测试架构。该架构通过多种测试类型确保代码的正确性、稳定性与性能。本文档旨在揭示 SynapseDB 的测试体系，涵盖从单元测试到系统级验证的完整链条，并指导开发者如何遵循现有规范贡献高质量测试。

## 测试分层策略

SynapseDB 采用多层级测试策略，覆盖不同粒度和场景，确保软件质量在各个层面都得到保障。

### 单元测试：验证独立函数逻辑
单元测试聚焦于最小可测单元，如工具类 `MinHeap` 和暂存机制 `LsmLiteStaging`，验证其核心行为是否符合预期。

例如，`MinHeap` 类实现了最小堆数据结构，用于高效管理优先级队列。其单元测试验证了插入、弹出、查看最小值等操作的正确性以及堆序性质的维持。

```mermaid
flowchart TD
A[创建 MinHeap 实例] --> B[push(5), push(3), push(7)]
B --> C[peek() 应返回 3]
C --> D[pop() 应返回 3]
D --> E[size() 应为 2]
E --> F[连续 pop 应按升序返回: 3,5,7]
```

**Diagram sources**
- [minHeap.ts](file://src/utils/minHeap.ts#L4-L113)
- [minHeap.test.ts](file://tests/unit/storage/minHeap.test.ts#L1-L28)

类似地，`LsmLiteStaging` 类实现了一种轻量级 LSM 树内存暂存策略，支持记录添加、大小查询和批量清空（drain）。其单元测试覆盖了基础操作、泛型支持、边界条件及内部状态一致性。

```mermaid
flowchart TD
G[创建 LsmLiteStaging 实例] --> H[add('record-1'), add('record-2')]
H --> I[size() 应为 2]
I --> J[drain() 应返回 ['record-1','record-2']]
J --> K[size() 应为 0]
K --> L[再次 drain 应返回空数组]
```

**Diagram sources**
- [staging.ts](file://src/storage/staging.ts#L14-L29)
- [staging.test.ts](file://tests/unit/storage/staging.test.ts#L1-L270)

此外，针对空间索引模块中的 `RTree` 类也进行了充分的单元测试。`RTree` 实现了高效的R-Tree空间索引，支持边界框查询、最近邻搜索等功能。近期修复了插入过程中因分裂回溯不当导致的越界问题，确保了树结构的稳定性和查询准确性。

```mermaid
flowchart TD
M[创建 RTree 实例] --> N[插入点(0,0), (1,1), (2,2)]
N --> O[边界框查询[-1,-1,3,3]应返回全部]
O --> P[最近邻查询(0,0)应返回(0,0)]
P --> Q[范围查询距离1.5内应包含多个点]
Q --> R[删除点(1,1)后验证剩余结果]
```

**Diagram sources**
- [rtree.ts](file://src/spatial/rtree.ts#L160-L814)
- [rtree.test.ts](file://tests/unit/spatial/rtree.test.ts#L1-L34)

### 集成测试：覆盖模块间协作
集成测试关注多个模块协同工作时的行为，验证接口契约和数据流的正确性。例如，在查询与存储模块之间进行交互测试，确保 Cypher 查询能够正确访问由 WAL（Write-Ahead Log）持久化的数据。

这类测试通常涉及实际的磁盘 I/O、事务处理和索引更新，模拟真实使用场景下的组件交互。

**Section sources**
- [integration/query](file://tests/integration/query/)
- [integration/storage](file://tests/integration/storage/)

### 系统测试：检验端到端行为
系统测试从整体角度验证数据库的核心特性，包括事务隔离级别和故障恢复能力。

例如，`query_snapshot_isolation.test.ts` 验证快照隔离（Snapshot Isolation）语义是否正确实现，确保并发读写不会导致脏读或不可重复读。另一项关键测试是 `crash_injection.test.ts`，它通过注入崩溃来检验数据库能否在异常重启后保持数据一致性和持久性。

这些测试模拟生产环境中的极端情况，是保证数据库可靠性的最后一道防线。

**Section sources**
- [system/query_snapshot_isolation.test.ts](file://tests/system/query_snapshot_isolation.test.ts)
- [system/crash_injection.test.ts](file://tests/system/crash_injection.test.ts)

### 性能测试：监控关键路径开销
性能测试用于基线化和持续监控数据库的关键操作耗时，防止性能退化。

SynapseDB 包含多种性能测试用例，如 `performance_large_insert.test.ts` 评估大批量数据插入吞吐量，`property_index_performance.test.ts` 测量属性索引的查询延迟。这些测试帮助团队识别瓶颈并优化核心路径。

**Section sources**
- [performance/performance_baseline.test.ts](file://tests/performance/performance_baseline.test.ts)
- [performance/performance_large_insert.test.ts](file://tests/performance/performance_large_insert.test.ts)

## Vitest 框架配置与组织结构

SynapseDB 使用 [Vitest](https://vitest.dev/) 作为主要测试框架，因其速度快、TypeScript 支持良好且 API 与 Jest 兼容。

### vitest.config.ts 配置要点
主配置文件 `vitest.config.ts` 定义了测试运行环境的关键参数：

- **环境设置**：指定 `environment: 'node'`，适用于服务端数据库逻辑。
- **超时控制**：`testTimeout: 20000` 设置较长的超时时间，以适应涉及磁盘 I/O 的测试。
- **执行池策略**：使用 `pool: 'forks'` 并配置 `poolOptions.forks`，其中 `maxForks: 2` 限制并发进程数，降低内存压力；同时通过 `execArgv: ['--max-old-space-size=8192']` 增加每个进程的内存上限，应对 JIT 编译峰值。
- **串行执行**：`sequence.concurrent: false` 禁用并发测试执行，进一步减少资源竞争。
- **覆盖率配置**：设定语句、分支、函数和行数的覆盖率门槛分别为 80%、75%、80%、80%，并排除非核心逻辑文件（如 CLI、类型定义等）。
- **全局清理**：通过 `globalSetup` 配置项引入 `tests/setup/global-cleanup.ts`，在所有测试结束后自动清理临时数据库文件，确保测试环境的纯净。

此配置确保测试稳定运行，尤其适合对 IO 和资源敏感的数据库系统。

**Section sources**
- [vitest.config.ts](file://vitest.config.ts#L1-L68)

### 测试组织方式
测试用例按功能目录划分，结构清晰，便于定位和维护：

```
tests/
├── unit/           # 单元测试
│   ├── utils/
│   ├── storage/
│   └── query/
├── integration/    # 集成测试
│   ├── query/
│   ├── storage/
│   └── maintenance/
├── system/         # 系统测试
├── performance/    # 性能测试
└── types/          # 类型测试
```

近期提交中已将大型测试套件拆分，提升了可维护性。CI 工作流也进行了优化，确保测试流程更加稳健。

**Section sources**
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml#L1-L47)

## 编写新的测试用例

为 SynapseDB 贡献新测试应遵循以下模式：

### 模拟环境与构造测试数据
对于单元测试，通常无需外部依赖，直接实例化被测对象即可。对于涉及文件系统的测试，建议使用临时目录隔离副作用。

**新增测试助手模块 `tempfs`**  
根据最新提交，项目引入了 `tests/helpers/tempfs.ts` 模块，提供统一的测试环境管理功能：
- `makeWorkspace(prefix)`：在操作系统临时目录中创建带有前缀的隔离工作区
- `cleanupWorkspace(dir)`：递归删除指定工作区，避免污染仓库根目录
- `within(dir, ...paths)`：在工作区内拼接文件路径

该模块已在多个测试文件中应用，如 `lazy_explain_estimate.test.ts` 和 `snapshot_memory_optimization.test.ts`，确保所有测试产生的临时文件（如 `tmp-*.synapsedb` 及其关联的 `.pages`、`.wal`、`.lock` 文件）都在测试结束后被彻底清理。

```typescript
import { makeWorkspace, cleanupWorkspace, within } from '../../helpers/tempfs';

describe('测试示例', () => {
  let testDir: string;
  let testDbPath: string;

  beforeEach(async () => {
    testDir = await makeWorkspace('test-example');
    testDbPath = within(testDir, 'db.synapsedb');
  });

  afterEach(async () => {
    await cleanupWorkspace(testDir);
  });

  it('应能正确执行查询', async () => {
    // 测试逻辑
  });
});
```

**Section sources**
- [tempfs.ts](file://tests/helpers/tempfs.ts#L1-L41)
- [lazy_explain_estimate.test.ts](file://tests/unit/query/lazy_explain_estimate.test.ts#L1-L26)
- [global-cleanup.ts](file://tests/setup/global-cleanup.ts#L1-L28)

### 全局清理与CI保障
除了测试用例内的清理，项目还实现了两级保障机制：
1. **测试级别**：每个测试套件通过 `afterEach` 调用 `cleanupWorkspace` 清理其工作区。
2. **全局级别**：Vitest 的 `globalSetup` 机制在所有测试结束后执行 `global-cleanup.ts`，扫描并移除仓库根目录下所有残留的 `tmp-*.synapsedb` 文件。
3. **CI级别**：CI工作流中新增 `Assert no tmp synapsedb leftovers` 步骤，使用 `find` 命令严格检查构建后是否存在临时文件残留，确保任何遗漏的清理都会导致CI失败。

这种多层防护机制有效防止了测试数据的累积和环境污染。

**Section sources**
- [global-cleanup.ts](file://tests/setup/global-cleanup.ts#L1-L28)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml#L68-L81)

### 调整测试阈值以适应环境
某些系统测试（如内存使用测试）的阈值需要根据运行环境（如是否在覆盖率模式下）进行调整。例如，在 `snapshot_memory_optimization.test.ts` 中，通过 `process.env.VITEST_COVERAGE` 检测是否在生成覆盖率报告，并据此调整数据规模和内存增长的验收标准。

```typescript
const isCoverage = !!process.env.VITEST_COVERAGE;
const recordCount = isCoverage ? 2000 : 10000;

// ...

// 验收标准：内存增长 ≤ 13MB（覆盖率模式下存在额外开销）
expect(memoryGrowthMB).toBeLessThanOrEqual(13);
```

这种自适应的测试设计提高了测试的稳定性和可移植性。

**Section sources**
- [snapshot_memory_optimization.test.ts](file://tests/system/snapshot_memory_optimization.test.ts#L1-L201)

## 测试驱动开发的重要性
测试驱动开发（TDD）在数据库项目中尤为重要。通过先编写测试，开发者可以：
- 明确接口契约和预期行为
- 及早发现设计缺陷
- 确保代码变更不会破坏现有功能
- 提供详尽的回归测试套件

我们鼓励所有贡献者遵循现有测试规范，为新功能和修复编写相应的测试用例。

## 结论
SynapseDB 的测试架构是一个多层次、自动化且健壮的系统，它不仅验证了代码的正确性，还保障了系统的性能和可靠性。通过持续改进测试实践和工具链，我们致力于为用户提供一个值得信赖的数据库产品。