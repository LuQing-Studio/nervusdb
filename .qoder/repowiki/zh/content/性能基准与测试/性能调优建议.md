...
- [queryBuilder.ts](file://src/query/queryBuilder.ts) - *新增诊断与估算功能*
- [lazy_explain_estimate.test.ts](file://tests/unit/query/lazy_explain_estimate.test.ts) - *新增估算功能测试*
</cite>

## 更新摘要
- **新增诊断功能**：添加 `explain()` 方法支持查询执行摘要，集成 `SYNAPSEDB_TRACE_QUERY` 环境变量用于性能分析
- **新增基数估算**：利用热度数据 `hotnessPrimary` 进行查询基数粗略估算，为查询优化提供数据支持
- **新增测试用例**：添加 `lazy_explain_estimate.test.ts` 验证估算功能正确性
- **更新引用文件**：将 `queryBuilder.ts` 和新测试文件加入文档引用
...
最新版本引入了查询执行诊断和基数估算功能，为性能分析和查询优化提供更深入的洞察：

1. **执行摘要功能**：`QueryBuilder.explain()` 方法现在提供查询执行的详细摘要，包括类型、长度、方向性、前沿大小和是否固定等信息，便于调试和性能分析。

2. **环境变量诊断**：通过设置 `SYNAPSEDB_TRACE_QUERY=1` 环境变量，可以在执行期间输出各阶段的耗时与计数，帮助识别性能瓶颈。

3. **基数估算优化**：系统现在利用热度数据 `hotnessPrimary` 进行基数粗略估算，结合索引顺序、页数和热度信息，为查询优化器提供更准确的基数估计，从而生成更优的执行计划。

4. **估算信息包含**：估算摘要包含索引顺序、上界、主维度页数、热度信息和预计输出等关键指标，帮助开发者理解查询的预期性能特征。

这些新功能已在 `lazy_explain_estimate.test.ts` 测试用例中得到验证，确保其准确性和可靠性。

**Section sources**
- [persistentStore.ts](file://src/storage/persistentStore.ts#L799-L1665)
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L170-L178) - *explain() 方法实现*
- [queryBuilder.ts](file://src/query/queryBuilder.ts#L1271-L1350) - *hotnessPrimary 热度估算逻辑*
- [lazy_explain_estimate.test.ts](file://tests/unit/query/lazy_explain_estimate.test.ts) - *估算功能测试*
