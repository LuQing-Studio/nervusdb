...
pub(super) fn evaluate_temporal_function(name: &str, args: &[Value]) -> Option<Value> {
    match name {
        "date" => Some(construct_date(args.first())),
        "localtime" => Some(construct_local_time(args.first())),
        "time" => Some(construct_time(args.first())),
        "localdatetime" => Some(construct_local_datetime(args.first())),
        "datetime" => Some(construct_datetime(args.first())),
