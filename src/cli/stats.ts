...
async function stats(
  ...
  // 属性索引统计
  let propertyStats: any = undefined;
  ...
  // 热度统计
  let hotnessStats: any = undefined;
  ...
  // 读者注册信息
  let readersStats: any = undefined;
  ...
  // 数据库文件大小统计
  let fileSizeStats: any = undefined;
  if (opts.verbose) {
    try {
  ...
  // 添加新的统计信息
  if (propertyStats) out.propertyIndex = propertyStats;
  if (hotnessStats) out.hotness = hotnessStats;
  if (readersStats) out.readers = readersStats;
  if (fileSizeStats) out.fileSizes = fileSizeStats;
...
async function main() {
  ...
  if (!dbPath) {
    ...
    console.log(
      '  --all                  包含所有扩展统计（等同于 --property --hotness --readers --verbose）',
    );
